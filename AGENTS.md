...
- **PITFALL Android `bg-primary/10` + `text-primary` (iconos invisibles):** las utilidades con opacidad (`bg-primary/10`, `/15`, `/20`, …) compilan a `background-color: var(--primary)` **sólido naranja** como fallback en navegadores/WebViews Android que NO soportan `color-mix()`. Si además el contenido usa `text-primary` (naranja), el icono/tarjeta queda naranja sobre naranja → invisible (funciona en Chrome de escritorio, falla en móvil). Fix correcto: usar **fondo sólido + texto de alto contraste**: `bg-primary text-primary-foreground` (blanco) para chips/cajas de icono; NO combinar `bg-primary/*` (con opacidad) con `text-primary`. Aplicado en `src/app/(public)/page.tsx` (commit `f0d3330`); el patrón `bg-primary/10 text-primary` aún persiste en otras páginas públicas (contacto, transparencia, organo-ejecutivo, etc.) — migrarlas cuando se toque.
