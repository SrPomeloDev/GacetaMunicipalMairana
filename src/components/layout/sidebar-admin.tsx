...
function setCollapsedStored(v: boolean) {
  try {
...
export function useSidebarCollapsed(): [boolean, (v: boolean) => void] {
  ...
  return [collapsed, setCollapsedStored]
}

export default function SidebarAdmin({ open = false, onClose }: { open?: boolean; onClose?: () => void }) {
  const [collapsed, setCollapsedStored] = useSidebarCollapsed()
  ...
  const sidebarContent = (
    <div className={cn(
      ...
      <div className={cn(
        "flex h-16 shrink-0 items-center overflow-hidden border-b border-sidebar-border px-4",
        collapsed ? "justify-center" : "justify-between"
      )}>
        {!collapsed && (
          <Link href="/admin/dashboard" className="flex min-w-0 animate-in items-center gap-2.5 fade-in duration-300" onClick={onClose}>
            <Image
      ...
      <div className="shrink-0 border-t border-sidebar-border p-2.5">
        <Link
          ...
          className={cn(
            "group flex items-center gap-3 overflow-hidden rounded-xl px-3 py-2.5 text-sm font-medium transition-all",
            collapsed && "justify-center px-2",
            "text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground"
          )}
        >
          ...
          {!collapsed && <span className="truncate whitespace-nowrap animate-in fade-in duration-300">Ver sitio</span>}
        </Link>
      </div>
      ...
      <div className={cn(
        ...
        <div className={cn("flex items-center gap-3 rounded-xl bg-muted/40 p-2", collapsed && "flex-col bg-transparent p-0")}>
          ...
          {!collapsed && (
            <Link href="/admin/perfil" className="group min-w-0 flex-1 animate-in fade-in duration-300">
        ...
        {!collapsed && (
          <Button variant="ghost" size="sm" onClick={handleLogout} className="mt-2 w-full animate-in justify-start fade-in text-muted-foreground duration-300 hover:bg-destructive/10 hover:text-destructive">
